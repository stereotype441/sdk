...
''');
  ...
  test_functionExpression_dynamic_context() async {
    await analyze('''
_f() => () => (x) => 0;
''');
...
Object Function(_C) _f() => (_C x) => x._g();
''');
    visitSubexpression(
        findNode.functionExpression('(_C x)'), 'String Function(_C)',
...
''');
  ...
  test_functionExpression_infer_parameter_type_match_named() async {
    await analyze('''
abstract class _C {
  String _g();
}
Object Function({_C x}) _f() => ({x}) => x._g();
''');
    visitSubexpression(
        findNode.functionExpression('({x})'), 'String Function({x: _C})',
        contextType: _migratedReturnType('_f'));
  }

  test_functionExpression_infer_parameter_type_match_optional() async {
    await analyze('''
abstract class _C {
  String _g();
}
